...
public class MeanLogger {
     ...
     */
    public static void addDataReceiver(MeanLogDataReceiver dataReceiver) {
        if (!running) {
                ...
                if (urclSupplier != null && RobotBase.isReal()) {
                    ...
                    if (buffers.length == 3) {
                        for (int i = 0; i < 3; i++) {
                            ...
                            switch (i) {
                                case 0:
                                    entry.put("URCL/Raw/Persistent", new MeanLogTable.LogValue(bytes, "URCLr3_persistent"));
                                    break;
                                case 1:
                                    entry.put("URCL/Raw/Periodic", new MeanLogTable.LogValue(bytes, "URCLr3_periodic"));
                                    break;
                                case 2:
                                    entry.put("URCL/Raw/Aliases", new MeanLogTable.LogValue(bytes, "URCLr3_aliases"));
                                    break;
                            }
                        }
                    }
...
//                    (periodicBeforeLength + userCodeLength + periodicAfterLength) / 1000.0);
            ...
            try {
                ...
                receiverQueue.add(MeanLogTable.clone(entry));

...
class PowerTracker {
    ...
    fun addMotors(name: String, currentSupplier: () -> Double, numMotors: Int = 1) {
        motors.add(MotorTracker(name, currentSupplier, numMotors))
    }

    fun update(currentVoltage: Double) {
        Logger.recordOutput("PowerTracker/Battery Voltage", currentVoltage, "volts")
        motors.forEach { it ->
...
private class MotorTracker(val name: String, val currentSupplier: () -> Double, val numMotors: Int = 1) {
    var totalCharge = 0.0
    var totalPower = 0.0

    fun update(currentVoltage: Double) {
        val current = numMotors * currentSupplier.invoke()
        val power = currentVoltage * numMotors * currentSupplier.invoke()
